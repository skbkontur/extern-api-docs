...
<a name="3"></a>
# Доверительная 
Метод аутентификации, позволяющий получить auth.sid, используя аутентификацию пользователя в доверенной системе Компании-Партнера. Для того, чтобы воспользоваться доверительной аутентификацией необходимо при первом входе пользователя произвести связывание пользователей  систем Партнера и Контура.
...
* подписывает данные запроса своим ключом
