...
При вызове метода необходимо указать значение ``deferred=true``. Для выполнения метода будет создана задача (Task). Статус ее выполнения необходимо смотреть по taskId. 

.. warning:: Для значения флага ``deferred=false`` вернется ошибка 400 BadRequest, так как значение false устарело. 
