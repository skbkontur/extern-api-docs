...
Также в Пенсионный Фонд России можно отправить другие формы отчетности через систему ЭДОК:
  ...
  * СЗВ-ТД.
  ...
  Подробнее про данные документообороты см. в статье :doc:`Отчетность в ПФР по форме СЗВ-ТД</PFR/szvtd>`
...
1. Если нет файла описи:
   ...
   * Создаем :doc:`DraftsBuilder</device_api/DraftsBuilder>` с типом urn:drafts-builder:pfr-report, в метаинформации обязательно заполнить в RecipientInfo поле ``upfr-code``, в AccountInfo поле ``registration-number-pfr`` — это информация, которая должна быть передана в УПФР с отчетом. 
