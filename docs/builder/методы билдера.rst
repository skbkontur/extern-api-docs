...
* ``builder-data``  — данные для указанного типа DraftsBuilder. Состав полей будет отличаться для каждого типа DraftsBuilder. 
   ...
   .. tabs::
      ...
      .. tab:: тип fns534-letter
         ...
         Тип DraftsBuilder fns534-letter предназначен для формирования файла обращения, чтобы отправить письма в ФНС с обращением и приложенными файлами.

         При создании DraftsBuilder в теле запроса обязательно заполняется параметр ``region``.

         Если требуется отправить ответ на входящее письмо ФНС, то обязательно заполнение параметра ``related-document``.
