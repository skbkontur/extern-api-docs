...
Для отправки большого количества документооборотов рекомендуется реализовать очередь отправок с конфигурацией количества параллельных запросов.
...
В ответе каждого запроса будет лежать заголовок ``X-Kontur-Trace-Id`` — уникальный идентификатор запроса. Рекомендуем сохранять или логировать на вашей стороне данный идентификатор для диагностики работы вашего приложения.
...
Ленту событий удобно использовать интеграторам, которые работают со множеством учетных записей Контур Экстерна. В такой ситуации у вас будет единый механизм получения уведомлений по изменениям статусов документооборотов для всех учетных записей.

Список документооборотов удобно использовать, когда в интеграции у вас одна учетная запись. В таком сценарии достаточно будет вычитывать список документооборотов.
...
Чтобы получить документообороты (ДО), нужно использовать метод :ref:`GET Docflows<rst-markup-get-dcs>`. При этом нужно соблюдать следующее ограничение:
...
При получении общего списка документооборотов без фильтра type метод GET Docflows не возвращает документообороты следующих типов:
...
После выбора из списка нужного документооборота получите его по идентификатору. В ответе вернется информация о документообороте и документах в нем. Не рекомендуется использовать отдельные методы получения документов по идентификатору, вся информация о них содержится в документообороте.

Отложенное выполнение задач
~~~~~~~~~~~~~~~~~~~~~~~~~~~

Рекомендуем использовать в API отложенное выполнение задач в методах, где это предусмотрено. Т.е. использовать флаг deferred=true, например в методах:
...
Некоторые операции из этого списка потенциально выполняются дольше других методов в API, например, более 10-15 минут. Данные методы обрабатывают контенты, поэтому длительность работы также зависит от их размера. 
