...
**Как отправить реестр ПВСО**
    ...
    5. Когда черновик готов, запустите последовательность методов: `POST Check`_ -> `POST Prepare`_ -> `POST Send`_. Подпись и документ объединит `POST Prepare`_.
...
**Как отслеживать документооборот**
    ...
    .. note:: Для поиска документооборота с фильтром ``type=fss-sick-report`` не используйте параметры ``updatedFrom`` и ``updatedTo``. Из-за особенностей документооборота с ФСС результатов поиска по дате и времени отправки ДО не будет.

    2. Запросите документооборот по идентификатору: `GET Docflow`_.

**Как получить результат обработки реестра ПВСО**

Результат документооборота можно определить по :ref:`стадиям и их статусам<rst-markup_4fss>`. Для этого проверьте поле ``discription`` и параметры ``fss-stage-type`` и ``fss-stage-status``:

1. если стадия **FomingReceipt** имеет статус **success**, поле ``documents`` будет содержать документ с типом urn:document:fss-sick-report-receipt;
...
2. если любая из стадий имеет статус **error**:
    ...
    * параметр ``fss-stage-error-expend`` вернет описание ошибки в html-формате;
...
Чтобы скачать документ, воспользуйтесь :doc:`Сервисом контентов</contents/content_methods>`. Посмотреть идентификатор документа вы можете в параметрах ``docflow-document-contents`` и ``content-id``.
