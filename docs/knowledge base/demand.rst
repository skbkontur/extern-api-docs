...
* по номеру требования: ``demandDocument``.
...
Во втором случае, метод сможет найти по номеру только распознанное требование. После того, как был получен список требований, по их идентификаторам ``docflowId`` можно узнать номер методом :ref:`распознования входящего требования<rst-markup-document-recognize>`. 
