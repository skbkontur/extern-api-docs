...
Для некоторых документооборотов, например, при ответе на требование о предоставление пояснений, нужно прикладывать счета-фактуры. Счет-фактура состоит из формализованного xml-файла и sgn-файла подписи контрагента. При работе с DraftsBuilder необходимо прикладывать оба файла. 

Если подписи контрагента нет, то необходимо приложить файл подписи отправителя в формате sgn.

 .. image:: /_static/DB_sf.png
     :width: 800px

Файлы в каждом черновике нужно подписывать КЭП отправителя описи. Это можно сделать перед отправкой черновика. Либо загрузить а документ DraftsBuilder уже подписанные файлы. И перед отправкой черновика нужно будет скачать, подписать и заново загрузить все неподписанные файлы и сформированный файл описи.

**Отправка накладных**

Для некоторых документооборотов необходимо прикладывать накладные. Накладная состоит из следующих файлов: 

1. Хml-файл продавца.
2. Sgn-файл подписи продавца.
3. Xml-файл покупателя.
4. Sgn-файл подписи покупателя.

В будущий черновик нужно приложить все четыре файла, на основе которых сформируется опись. 

Если подписи контрагента нет, то необходимо приложить файл подписи отправителя в формате sgn.

 .. image:: /_static/DB_nak.png
     :width: 800px

Файлы в каждом черновике нужно подписывать КЭП отправителя описи. Это можно сделать перед отправкой черновика. Либо загрузить в документ уже подписанные файлы. И перед отправкой черновика нужно будет скачать, подписать и заново загрузить все неподписанные файлы и сформированный файл описи.

**Отправка неформализованных файлов**

Такие неформализованные файлы, как сканы, как правило, имеют формат jpg. Для работы с DraftsBuilder необходимо будет добавить также sgn-файл подписи контрагента. 

Если подписи контрагента нет, то необходимо приложить файл подписи отправителя в формате sgn.

 .. image:: /_static/DB_skan.png
     :width: 800px

Файлы в черновике нужно подписывать КЭП отправителя описи. Это можно сделать перед отправкой черновика. Либо загрузить в документ уже подписанные файлы. Если файлы сканов не соответствуют требованиям КО, то DraftsBuilder приведет файлы к установленному формату. В таком случае, подпись может стать невалидной и перед отправкой черновика потребуется файл скачивать, подписывать и загружать заново. 

**Когда не нужно прикладывать подпись**

Прикладывать подпись не нужно для следующих форм КНД:
