.. _`поиск документооборотов`: https://developer.kontur.ru/doc/extern/method?type=get&path=%2Fv1%2F%7BaccountId%7D%2Fdocflows
...
* В свойствах документа в документообороте реализован новый параметр SupportPrint, который поможет определить возможность печати документа. Параметр может иметь одно из трех значений: no, yes, unknown. Подробнее в статье :doc:`</knowledge base/print>`.

.. **Обновления в Extern Test Tools:**
.. * Поддержан новый способ :doc:`аутентификации по протоколу OpenId Connect </auth_oidc/index>`. Старый способ аутентификации через auth.sid будет поддерживаться для реализованных интеграций. 
