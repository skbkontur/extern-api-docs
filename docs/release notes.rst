...
-------------
    ...
    * Добавили описание :doc:`аутентификации приложения через Client Credentials Flow</auth_oidc/client_credentials_flow>` для взаимодействия между сервисами без участия пользователя.
    * Для :doc:`отправки отчетности в Росстат</scenarios/Rosstat/stat-report>` уточнили заполнение ``additional-info.warrant-info``: необходимо указать все три поля объекта.

Август 2026
-----------

    * Добавили :ref:`статусы ленты событий<rst-markup-event-state>` для документооборота регистрации или отзыва доверенности СФР ``urn:docflow:fss-warrant-management``.

Май 2026
--------

    * Добавили :doc:`сценарий работы с документом о предстоящей поставке товара (ДОПП) в системе СПОТ</scenarios/FNS/report/spot>`: создание и отправку черновика, обработку ответных документов и отслеживание документооборота.


Декабрь 2025
-------------

    * Поддержали работу с документооборотом :ref:`Сведения, необходимые для исчисления страхователем первых 3 дней пособия по временной нетрудоспособности<rst-markup-typeDo-sedo>`. 

Ноябрь 2025
-----------

    * Поддержали работу с документооборотами :ref:`Запрос информации о судебном ограничении<rst-markup-typeDo-sedo>` и :ref:`Ответ на запрос информации о судебном ограничении<rst-markup-typeDo-sedo>`. 
