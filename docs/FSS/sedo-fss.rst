...
На текущий момент через данный сервис возможно работать с документооборотами, которые участвуют в схеме :doc:`Проактивных выплат</FSS/proactiv>`. К ним также относятся следующие документообороты:
...
Для получения извещений ПВСО и уведомлений об изменении статуса ЭЛН, сперва необходимо совершить :doc:`подписки на получение извещений и уведомлений</FSS/sedo-subscription>`. Подписки также являются отдельными документооборотами. 
