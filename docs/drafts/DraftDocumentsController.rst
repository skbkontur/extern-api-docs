...
``deferred`` — флаг :ref:`асинхронного выполнения запроса<rst-markup-print-async>`. Запрос будет выполнен асинхронно, что позволит не ожидать завершения выполнения запроса. При вызове метода необходимо указать значение флага *true*. Будет создана задача (Task). Статус ее выполнения необходимо смотреть по task-Id. 
