...
.. warning:: Методы ``Check``, ``Prepare``, ``Send`` необхоидмо выполнять :ref:`асинхронно<rst-markup-deferred_draft>`. При таком вызове будет создана задача, статус выполнения которой можно отслеживать по taskId. 
