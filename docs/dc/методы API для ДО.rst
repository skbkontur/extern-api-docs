...
``content-id`` — расшифрованный и разжатый контент. В данном параметре нужно передать идентификатор content-id, полученный после загрузки расшифрованного разжатого контента в :doc:`сервис контентов</contents/index>`. Большие контенты следует загружать в сервис контентов и использовать асинхронный запрос на печать. 
